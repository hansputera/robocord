...
export class Context extends MessageClassRest {
  ...
  async sendAPI(options?: RESTPostAPIChannelMessageJSONBody): Promise<Context> {
    try {
      const resp = this._client._rest.api.post(
      ...
      return new Context(
        this._client,
        new MessageClassRest(this._client, (await resp.json()) as APIMessage)
      );
    } catch (err) {
  ...
  async edit(content: string, options?: RESTPatchAPIChannelMessageJSONBody) {
    ...
    try {
      const resp = this._client._rest.api.patch(
      ...
      return new Context(
        this._client,
        new MessageClassRest(this._client, json as APIMessage)
      );
    } catch {
      return undefined;
    }
  ...
  private async _delete() {
    if (this.author.id != this._client.user.id) return undefined;
    try {
      await this._client._rest.api.delete(
        'channels/' + this.channelID + '/messages/' + this.id
      );
    } catch {
      return undefined;
