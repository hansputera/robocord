...
export class MessageEvent extends BaseEvent {
  ...
  async onCreate() {
    const message = new MessageClassRest(
      this.client,
      this.raw.d as unknown as APIMessage
    );
  ...
  onEdit() {
    ...
    this.client.emit(
      'updateMessage',
      oldMessage,
      new Context(this.client, message)
    );
  }

  onDelete() {
    const oldMessage = this.messages.get(
      (this.raw.d as Record<string, string>).id
    );
