...
export class RZRWebSocket {
  ...
  constructor(
    ...
    if (existsSync(path.resolve(client.getOptions().sessionFile))) {
      const context = readFileSync(
        path.resolve(client.getOptions().sessionFile),
        {
          encoding: 'utf-8',
        }
      );
  ...
  resume(emitted = true): void {
    if (!this.session.length)
      throw new Exception(
        'INVALID_SESSION',
        'Websocket session token is empty'
      );
    this.connectionType = 'reconnecting';
    if (emitted) this.client.emit('reconnect');
    ...
    ws.on('open', () => {
      ...
      .on('close', (code, reason) => {
        ...
        this.client.emit(
          'close',
          code,
          uncompressReason && uncompressReason.toString('utf8')
        );
  ...
  public send(
    ...
    if (this.ws) {
      ...
      throw new Exception(
        'WEBSOCKET_OFFLINE',
        "Websocket not connected, couldn't send anything to discord"
      );
    }
  }
    ...
    try {
      ...
      if (parses.t === 'READY') {
        ...
        this.client.user = new ClientUser(
          this.client,
          parses.d.user as APIUser
        );
        this.startedConnect = new Date().getTime();
        if (existsSync(path.resolve(this.client.getOptions().sessionFile)))
          writeFileSync(
            path.resolve(this.client.getOptions().sessionFile),
            parses.d.session_id as string
          );
        ...
        if (events.length) {
          events.forEach((event) => {
            const diffSeconds =
              (new Date().getTime() - this.startedConnect) / 1000;
