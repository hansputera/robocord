...
export class EmbedBuilder {
    ...
    static resolveColor(color: KEYWORD | HSL | XYZ): number {
        ...
        if (typeof color === 'string') {
            const colorName = Color.keyword.rgb(color as KEYWORD);
            const colorHex = Color.hex.rgb(color as string);

            if (colorName) result = EmbedBuilder.convertRgb(colorName);
            else if (!colorName && colorHex) result = EmbedBuilder.convertRgb(colorHex);
        } else if (Array.isArray(color)) {
            const colorHSL = Color.hsl.rgb(color as HSL);
            const colorXYZ = Color.xyz.rgb(color as XYZ);

            if (colorHSL) result = EmbedBuilder.convertRgb(colorHSL);
            else if (!colorHSL && colorXYZ) result = EmbedBuilder.convertRgb(colorXYZ);
        }
    ...
    setColor(color: string | HSL | XYZ) {
        const resolvableColor = EmbedBuilder.resolveColor(color as KEYWORD | HSL | XYZ);
